...
/datum/reagent/drug/zvezdochka/on_mob_metabolize(mob/living/M)
	...
	var/sound/sound = sound(pick('modular_bluemoon/sound/voice/LYENEN.ogg', 'modular_bluemoon/sound/voice/LYENEN2.ogg'), TRUE)
...
/datum/reagent/drug/zvezdochka/on_mob_life(mob/living/M)
	...
	new /datum/hallucination/delusion(M, TRUE, "custom", rand(10, 50), custom_icon_file = 'modular_bluemoon/icons/effects/stars.dmi', custom_icon = pick("star1","star2"), custom_name = "ЗВЁЗДОЧКА")
...
/datum/reagent/drug/pendosovka/on_mob_metabolize(mob/living/M)
	...
	var/sound/sound = sound(pick('modular_bluemoon/sound/voice/CAPITAL1.ogg', 'modular_bluemoon/sound/voice/CAPITAL2.ogg'), TRUE)
...
/datum/reagent/drug/pendosovka/on_mob_life(mob/living/M)
	...
	new /datum/hallucination/delusion(M, TRUE, "custom", rand(10, 50), custom_icon_file = 'modular_bluemoon/icons/effects/stars.dmi', custom_icon = pick("star1", "star3"), custom_name = "ПЕНДОСОВКА")
...
/datum/reagent/drug/labebium/on_mob_life(mob/living/carbon/H)
	...
	switch(current_trip)
			...
			switch(current_cycle)
				if(1 to 20)
					...
					if(prob(30))
						...
						H.cultslurring++
					if(!tripsoundstarted)
						var/sound/sound = sound('modular_bluemoon/sound/ambience/lifeweb/cometodaddy.ogg', TRUE)
				...
				if(31 to INFINITY)
					...
					if(prob(85))
						...
						for(var/atom/movable/screen/plane_master/whole_screen in screens)
							if(prob(3))
								var/sound/sound = sound('modular_bluemoon/sound/effects/trip_blast.wav')
								sound.environment = 23
								sound.volume = 100
								SEND_SOUND(H.client, sound)
								H.emote("realagony")
		...
		if("statues")
			...
			if(!tripsoundstarted)
				var/sound/sound = sound('modular_bluemoon/sound/ambience/lifeweb/caves8.ogg', TRUE)
				sound.environment = 23
				sound.volume = 80
				SEND_SOUND(H.client, sound)
				H.hud_used.show_hud(HUD_STYLE_NOHUD)
...
/obj/effect/hallucination/simple/water
	...
	image_icon = 'modular_bluemoon/icons/obj/lifeweb/water.dmi'
	image_state = "water0"
	...
	if(!triggered_shit)
		if(ishuman(AM))
			...
			if(prob(10) && AM == target)
				...
				var/sound/sound = sound('modular_bluemoon/sound/effects/pshsh.ogg')
				sound.environment = 23
				sound.volume = 20
				SEND_SOUND(M, sound)
				M.Paralyze(25)
...
/obj/effect/hallucination/simple/ovoshi
	name = "Овощ"
	desc = "М-М-М-М-М-М..!"
	image_icon = 'modular_bluemoon/icons/obj/lifeweb/harvest.dmi'
...
/obj/effect/hallucination/simple/ovoshi/attack_hand(mob/living/carbon/C)
	...
	var/sound/sound = sound(pick('modular_bluemoon/sound/ambience/lifeweb/wallhit.ogg', \
		'modular_bluemoon/sound/ambience/lifeweb/wallhit2.ogg', 'modular_bluemoon/sound/ambience/lifeweb/wallhit3.ogg'))
	sound.environment = 23
...
/obj/effect/hallucination/simple/ovoshi/statues
	name = "Мяу"
	desc = "Ого!"
	image_icon = 'modular_bluemoon/icons/obj/lifeweb/crafts.dmi'
	image_state = "statue1"
